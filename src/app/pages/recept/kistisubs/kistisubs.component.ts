...
@Component({
  selector: 'app-kistisubs',
  templateUrl: './kistisubs.component.html',
  styleUrls: ['./kistisubs.component.css']
})
export class KistisubsComponent {
id!: number;
...
loadBorrowerData(id: number) {
  this.loading = true;
  this.accountservice.getkistireceiveById(id).subscribe({
    next: (res) => {
      if (res && res.length > 0) {
